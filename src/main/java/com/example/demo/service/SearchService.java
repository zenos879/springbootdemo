...
public class SearchService {
    ...
    public String getCategory() {
        ...
        List<CategoryInfo> topCatList = null;
        Map<Integer,List<CategoryInfo>> lowerCatMap = new HashMap<Integer,List<CategoryInfo>>();
        if (topCatList == null || topCatList.size() == 0) {
            topCatList = searchDao.selectTopCate();
        }
        if (topCatList != null && topCatList.size() > 0) {
            for (int i = 0; i < topCatList.size(); i++) {
                ...
                if (lowerCatMap.size()==0 || lowerCatMap.get(catId)==null) {
                    List<CategoryInfo> listTmp = searchDao.selectLowerCat(catId);
                    lowerCatMap.put(catId,listTmp);
                }
                JSONArray lowerCatArr = new JSONArray();
                List<CategoryInfo> list = lowerCatMap.get(catId);
                if (list != null && list.size() > 0) {
                    for (int j = 0; j < list.size(); j++) {
                        CategoryInfo lowerRet = list.get(j);
